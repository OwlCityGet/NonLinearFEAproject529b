...
%       'plastic domain'
      ...
      dep=sqrt(1.5)*depBar*(Mdvsig-Myscold)/(EucNorm(Mdvsig-Myscold))^2;

      %MepijAcum=MepijAcum+dep;
      Mdvsig=Mdvsig-sqrt(6)*mu*depBar*(Mdvsig-Myscold)/(EucNorm(Mdvsig-Myscold));
      %if (ihard==1)
      Myscold=Myscold+Hbar*(1-bet)*dep;
      if bet==1
      akpold=sqrt(1.5*(EucNorm(Mdvsig-Myscold))^2);
      else
      akpold=6.0*10^4;
