...
%   akpold
 ...
 if (mstat==2) 
      ...
      %Mdvsig=Vec_Mat(dvsig);
      dep=sqrt(1.5)*depBar*(t)/EucNorm(Mdvsig-Myscold);
       
      
      MepijAcum=MepijAcum+dep;
      Mdvsig=Mdvsig-sqrt(6)*mu*depBar*(t)/EucNorm(t);
      if (ihard==1)
      ...
      if (ihard==1)
        be=1.5*(1/akpnew)^2*(1+(2/3)*((young*youngp)/(young-youngp))*((1+p)/young))^-1;
        a11=(1-p)/(1-2*p);
        a22=(p)/(1-2*p);
        S=Mdvsig-Myscnew;
